...
(* Maximum number of frames (time steps) before giving up on reaching
   quiescence *)
let cMAXFRAMES = 2000 ;;
...
(* solve initialize initialize_update render finalize 
   ...
   Solves a force-directed graph-drawing problem defined by masses and
...
let solve (initialize : unit -> unit)
  ...
  let rec update temperature prev_energy iters_left =
    try
      ...
      (* calculate forces, scale them, and update positions *)
      ...
      (* calculate new energy by summing energies of the constraints *)
      let energy =
        constraints
	...
	(* energy decreased; render a frame of the animation and keep
  	   going *)
