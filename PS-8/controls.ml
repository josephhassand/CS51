...
(* Class control --
   ...
   Controls are individual components of a graph that apply a force to
   one or more masses. At any given moment, the control contributes a
   certain amount of *energy*, and the derivative of that energy
   function is the force. In addition, the controls may have graphical
   means for revealing or describing the state of the control for
   debugging purposes.
...
(* Class position -- A control that pulls a mass m to a given fixed
   position p as an s/r^d force where s is the "stiffness"

   Arguments: ?stiffness : float     The stiffness of the spring
   	      ?degree : float	     The degree (d) of the restorative force
	      m : mass		     The mass being affected
	      p : point		     The point it's pulled towards
 *)
...
(* Class positionspring -- A control that pulls a mass m to a given
   position p as a Hooke's law spring with zero rest length and spring
   constant given by (stiffness dist) where dist is the distance from
   the mass to the point.

   Arguments: ?stiffness : float -> float    
   	      		   	     The stiffness of the spring as a function
				     of the distance between m and p
	      m : mass		     The mass being affected
	      p : point		     The point it's pulled towards
 *)
...
(* Class logisticrepel -- A repulsive force between two masses that
   pushes on the masses to place them at a fixed distance form each
   other, the rest length. The force on the two masses is modeled as a
   logistic function:

			  /                    1                 \
      force = stiffness * |1 - ----------------------------------|
			  |         steepness * (distance - rest)|
			  \    1 + e                             /

   Arguments: ?stiffness : float -> float    
   	      		   	The stiffness of the force; see equation
	      ?rest : float 	The rest length of the control, at which 
	      	      		the force is zero
	      ?steepness	See formula
	      m1 : mass		The masses being affected
	      m2 : mass
 *)
...
(* Class bispring -- A control that joins masses m1 and m2 with a
...
(* Class align -- A control that joins masses m1 and m2 with a
   ...
   Arguments: ?stiffness : float -> float    
   	      		   	The stiffness of the force
	      ?dir : direction 	The alignment direction
	      m1 : mass		The masses being affected
	      m2 : mass
*)  
...
(* Class equidistant -- A control that causes the distance between masses
   m1 and m2 to equilibrate with the distance between m3 and m4.

   Arguments: ?stiffness : float -> float     The stiffness of the force
	      m1, m2, m3, m4 : mass           The masses being affected
 *)
