...
let rec alternating_stream (first_stream: 'a stream) (second_stream: 'a stream) : 'a stream =
  fun () -> Cons (head first_stream, alternating_stream second_stream (tail first_stream))
...
let rec sfilter (cond : 'a -> bool) (input: 'a stream) : 'a stream =
  if cond (head input) then fun () -> Cons (head input, sfilter cond (tail input))
    else
      sfilter cond (tail input) ;;
...
let rec sieve (s : 'a stream) : 'a stream =
  fun () -> Cons(head s, sieve(sfilter (fun x -> not_div_by (head s) x) s))  ;;
